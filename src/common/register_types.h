...
namespace mmio_functions {
    ...
    static void set_int(T* const memory, unsigned owned_bits, unsigned value) {
        *memory = (*memory & ~owned_bits) | (value << __builtin_ctz(owned_bits));
    };
}
...
private:
    static constexpr unsigned shift_amount = __builtin_ctz(owned_bits);
};
...
private:
    constexpr static typename mmio::datatype _bitmask(access_type index) {
        return 1 << (typename mmio::datatype) index << __builtin__ctz(owned_bits);
    }
};
